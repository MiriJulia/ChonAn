from PySide6.QtWidgets import (
    ...
    QStackedWidget, QLineEdit, QRadioButton, QButtonGroup, QHBoxLayout, QMessageBox, QFrame, QSizePolicy, QScrollArea, QTextEdit
)
...
PROVIDERS = []
SERVICE_CLAIMS = []
# Sample member data for prototype
MEMBERS = [
...
# Sample service directory for prototype
SERVICE_DIRECTORY = [
...
class SignInPage(QWidget):
    ...
    def try_signin(self):
        ...
        if not user:
            self.show_signin_error("Invalid username.")
            return

        if user['password'] != password:
            self.show_signin_error("Incorrect password.")
            return

        if user['role'] != role:
            self.show_signin_error(f"User is not a {role}.")
            return
...
class AddProviderPage(QWidget):
    def __init__(self, main_window):
        ...
        self.main_window = main_window
        layout = QVBoxLayout()
        layout.addWidget(QLabel("Add New Provider"))
        self.entries = {}
        ...
        submit_btn.setStyleSheet(f"background: {CHOCOLATE}; color: {WHITE}; font-weight: bold;")
        submit_btn.clicked.connect(self.submit)
        layout.addWidget(submit_btn)
        back_btn = QPushButton("Back to Menu")
        back_btn.setStyleSheet(f"background: {CHOCOLATE}; color: {WHITE}; font-weight: bold;")
    ...
    def submit(self):
        ...
        if not all(data.values()):
            ...
            return
        provider_id = str(100000 + len(PROVIDERS) + 1)
        ...
        if confirm == QMessageBox.Yes:
            ...
            QMessageBox.information(self, "Success", f"Provider Added Successfully!\nProvider ID: {provider_id}")
...
class ServiceClaimPage(QWidget):
    def __init__(self, main_window):
        ...
        date_label = QLabel("Date of Service (MM-DD-YYYY):")
        ...
        self.date_input.setStyleSheet(f"background: {WHITE}; border: 2px solid {BARNEY}; border-radius: 10px; padding: 8px; font-size: 18px; color: black; min-width: 250px; max-width: 250px; outline: none;")
    ...
    def submit_claim(self):
        ...
        service = next((s for s in SERVICE_DIRECTORY if s['code'] == service_code), None)
        if not service:
            msg = QMessageBox(self)
            msg.setIcon(QMessageBox.Warning)
            msg.setWindowTitle("Error")
        ...
        claim_id = str(1000000 + len(SERVICE_CLAIMS) + 1)
        ...
        """)
        msg.exec()
        for entry in [self.member_id_input, self.date_input, self.provider_num_input, self.service_code_input, self.comments_input]:
            entry.clear()
...
class ProviderDirectoryPage(QWidget):
    ...
    def send_directory_email(self):
        ...
        sorted_services = sorted(SERVICE_DIRECTORY, key=lambda x: x['name'])
...
class ManageMembersPage(QWidget):
    def __init__(self, main_window):
        ...
        add_btn.setStyleSheet(f"background: {CHOCOLATE}; color: {WHITE}; font-weight: bold; border-radius: 10px; padding: 12px 24px; font-size: 16px; min-width: 300px;")
        add_btn.clicked.connect(self.add_new_member)
        buttons_layout.addWidget(add_btn)
        
        renew_btn = QPushButton("Renew Existing Member")
        renew_btn.setStyleSheet(f"background: {CHOCOLATE}; color: {WHITE}; font-weight: bold; border-radius: 10px; padding: 12px 24px; font-size: 16px; min-width: 300px;")
        renew_btn.clicked.connect(self.renew_member)
        buttons_layout.addWidget(renew_btn)
        
        modify_btn = QPushButton("Modify Member Information")
        modify_btn.setStyleSheet(f"background: {CHOCOLATE}; color: {WHITE}; font-weight: bold; border-radius: 10px; padding: 12px 24px; font-size: 16px; min-width: 300px;")
        modify_btn.clicked.connect(self.modify_member)
        buttons_layout.addWidget(modify_btn)
        
        remove_btn = QPushButton("Remove Member")
        remove_btn.setStyleSheet(f"background: {CHOCOLATE}; color: {WHITE}; font-weight: bold; border-radius: 10px; padding: 12px 24px; font-size: 16px; min-width: 300px;")
    ...
    def add_new_member(self):
        ...
        field_configs = [
            ...
            ("Member Number (9 digits):", "number", 9),
    ...
    def submit_new_member(self, fields, dialog):
        ...
        if not number.isdigit() or len(number) != 9:
            ...
            msg.setIcon(QMessageBox.Warning)
            msg.setWindowTitle("Error")
            msg.setText("Member number must be exactly 9 digits.")
            msg.setStyleSheet("""
                QLabel { color: black; }
                QPushButton { color: black; }
            """)
    ...
    def save_member_changes(self, member, fields, dialog):
        ...
        member['name'] = fields['name'].text().strip()
        member['address'] = fields['address'].text().strip()
        member['city'] = fields['city'].text().strip()
        member['state'] = state
        member['zip'] = zip_code
...
class ManageProvidersPage(QWidget):
    def __init__(self, main_window):
        ...
        modify_btn = QPushButton("Modify Provider Information")
        modify_btn.setStyleSheet(f"background: {CHOCOLATE}; color: {WHITE}; font-weight: bold; border-radius: 10px; padding: 12px 24px; font-size: 16px; min-width: 300px;")
        modify_btn.clicked.connect(self.modify_provider)
        buttons_layout.addWidget(modify_btn)
        
        delete_btn = QPushButton("Delete Provider")
        delete_btn.setStyleSheet(f"background: {CHOCOLATE}; color: {WHITE}; font-weight: bold; border-radius: 10px; padding: 12px 24px; font-size: 16px; min-width: 300px;")
    ...
    def add_new_provider(self):
        # Create a dialog for adding new provider
        dialog = QWidget()
        ...
        field_configs = [
            ...
            ("Provider Number (9 digits):", "number", 9),
            ("Street Address (25 chars):", "address", 25),
            ("City (14 chars):", "city", 14),
            ("State (2 letters):", "state", 2),
            ("ZIP Code (5 digits):", "zip", 5)
        ...
        submit_btn.setStyleSheet(f"background: {CHOCOLATE}; color: {WHITE}; font-weight: bold; border-radius: 8px; padding: 10px 20px;")
        submit_btn.clicked.connect(lambda: self.submit_new_provider(fields, dialog))
        button_layout.addWidget(submit_btn)
    ...
    def submit_new_provider(self, fields, dialog):
        # Validate all fields are filled
        for field_name, entry in fields.items():
            if not entry.text().strip():
                msg = QMessageBox(self)
        ...
        msg.setIcon(QMessageBox.Information)
        msg.setWindowTitle("Success")
        msg.setText(f"New provider added successfully!\n\nProvider ID: {number}\nName: {new_provider['name']}\n\nProvider information has been recorded.")
        msg.setStyleSheet("""
            QLabel { color: black; }
            QPushButton { color: black; }
        """)
    ...
    def save_provider_changes(self, provider, fields, dialog):
        ...
        provider['name'] = fields['name'].text().strip()
        provider['address'] = fields['address'].text().strip()
        provider['city'] = fields['city'].text().strip()
...
class MainWindow(QMainWindow):
    ...
    def goto_provider_directory(self, return_to_claim=False, is_manager=False):
        if return_to_claim:
            if self.provider_directory_claim is None:
            ...
            if self.provider_directory_menu is None:
